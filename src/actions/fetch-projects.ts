...
const finalAPI = projectLinks[Math.floor(Math.random() * projectLinks.length)];

export async function fetchCoinData(projectId: string) {
  const account_type: AccountType = "demo";
  const currency = "usd";
  const api = `https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=${projectId}&x_cg_demo _api_key=${finalAPI}`;
  //
...
export async function fetchCoinsData(currentPage?: number) {
  ...
  const api = `https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=50&page=${currentPage ?? 1}&x_cg_demo_api_key=${finalAPI}`; //
  const res = await fetch(api);
