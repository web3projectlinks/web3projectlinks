...
export async function fetchCoinsData() {
  ...
  const api = `https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=50&page=1&x_cg_demo_api_key=${process.env.NEXT_PUBLIC_COINGECKO_API_KEY}`; //
  const res = await fetch(api);
